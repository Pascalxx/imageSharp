...
def do_multiple(img_input_fst, img_input_sec):
    ...
    out_img = np.empty([img_input_fst.shape[0], img_input_fst.shape[1], 3])
...
def do_image_sharp(img_input, multiple_img_input):
    ...
    out_img = np.empty([img_input.shape[0], img_input.shape[1], 3])
    for i in range(img_input.shape[0]):
        for j in range(img_input.shape[1]):
            out_img[i][j] = img_input[i][j] / 255 + multiple_img_input[i][j]
...
if __name__ == '__main__':
    ...
    multiple_img = do_multiple(median_filter_img, laplacian_img)  # 一階模糊與二階影像相乘

    cv2.imshow('frame_o', img)
    do_image_sharp(img, multiple_img)
    cv2.waitKey(0)
